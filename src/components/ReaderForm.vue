<template>
    <Form @submit="submitReader" :validation-schema="readerFormSchema" :validate-on-mount="false">
        <div class="form-item col-md-6 offset-md-3 pb-3 my-3">
            <label class="form-label">Họ và tên</label>
            <Field type="text" class="form-control" name="name" v-model="readerLocal.name" />
            <ErrorMessage name="name" class="text-danger" />
        </div>

        <div class="form-item col-md-6 offset-md-3 pb-3 my-3">
            <label class="form-label">Số điện thoại</label>
            <Field type="text" class="form-control" name="phone" v-model="readerLocal.phone" />
        ...
        <div class="form-item col-md-6 offset-md-3 pb-3 my-3">
            ...
            <Field as="textarea" class="w-100 form-control mb-4" rows="5" cols="10" name="address"
        ...
        <div class="my-2 mx-4 d-flex justify-content-center">
            <button class="btn btn-primary text-white text-center" style="padding: 15px 45px; font-size: 1.25rem;">Đăng
                ký</button>
        </div>
    </Form>
</template>
...
export default {
    ...
    data() {
        const readerFormSchema = yup.object().shape({
            ...
            address: yup.string().required("Không được để trống").max(100, "Địa chỉ tối đa 100 ký tự."),
